...
def job(sign, CHAT_ID=CHAT_ID):
    ...
    text = client.chat.completions.create(
        ...
        messages=[
            ...
            { "role": "user", "content": f"Составь таро-гороскоп на {today} для знака '{sign}', используй смайлики, используй не более 300 слов" },
        ]
...
if __name__ == '__main__':
    schedule.every().day.at("09:00",'Europe/Moscow').do(job, sign="Рыбы", CHAT_ID='@pisces_the')
    schedule.every().day.at("09:01",'Europe/Moscow').do(job, sign="Овен", CHAT_ID='@aries_the')
    schedule.every().day.at("09:02",'Europe/Moscow').do(job, sign="Телец")
    schedule.every().day.at("09:03",'Europe/Moscow').do(job, sign="Близнецы", CHAT_ID='@gemini_the')
    ...
    schedule.every().day.at("09:10",'Europe/Moscow').do(job, sign="Козерог", CHAT_ID='@capricorn_the')
    schedule.every().day.at("09:11",'Europe/Moscow').do(job, sign="Водолей", CHAT_ID='@aquarius_the')
